...
cp -fv Runtime/Copy/docker-compose.yml  .
...
if [ "$Mode" == "uninstall" ]; then
    ...
    rm -fr build
    mkdir -pv build
    cp -fv Runtime/Copy/sources.list      build
    cp -fv Runtime/Setup-rpi/Dockerfile   build
    cp -fv Runtime/Setup-rpi/raspi.list   build
